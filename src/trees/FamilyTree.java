...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		else
		{
			...
			if(parentNode==null){
				throw new TreeException ("Parent not found");// .?????  There's a method in Node that searches for a named node. 
				}
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("Node not found");//??? Throw a TreeException with a useful message
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null)
			throw new TreeException("Node not found");//??? Throw TreeException with a useful message
